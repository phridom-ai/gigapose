...
print("Loading annotations from object_0.json...")
...
# Process only frames 0-59 (matching your rgb images)
for frame in data['frames'][:60]:
    frame_idx = frame['frame_idx']
    
    for inst in frame['instances']:
        # Only process detections for camera_idx 0
        if inst['camera_idx'] != 0:
            continue
        ...
        # First decode from list format to binary mask
        h, w = inst['segmentation']['size']
        counts = inst['segmentation']['counts']
